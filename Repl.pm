...
our $VERSION = sprintf("%d.%d", q$Revision: 1.27 $ =~ /(\d+)\.(\d+)/);
    ...
    if ( $mtime && $agelimit ) {
	   if ( $agelimit > 0) {  # if $agelimit is positive - ignore files older than the agelimit 
          if ( ( $runtime - $mtime ) > $agelimit ) {
            printf "%s - exceeds age limit (%3.1d days - limit is newer than %3d days))\n", $msg, (time - $mtime)/(86400), $agelimit/(86400) if ( $verbose > 1 );
            return FALSE;
          }
       }else{ #  if $agelimit is negative - ignore files newer than the agelimit
          if ( ( $runtime - $mtime ) > -$agelimit ) {
            printf "%s - exceeds age limit (%3.1d days - limit is older than %3d days))\n", $msg, (time - $mtime)/(86400), -$agelimit/(86400) if ( $verbose > 1 );
            return FALSE;
          }
       }
    }else{
...
to me at robertsdavea@googlemail.com. However, I can't promise to offer
